...
export function OrderStatusBadge({ status, type, className, showChevron }: OrderStatusBadgeProps) {
  ...
  return (
    <Badge
      variant="outline"
      className={cn("rounded-full border px-2.5 py-1 text-xs font-semibold uppercase tracking-wide inline-flex items-center gap-1", styles, className)}
    >
      {formatOrderStatusLabel(normalizedStatus)}
      {showChevron && <ChevronDown className="w-3 h-3 opacity-70" />}
    </Badge>
  );
}
