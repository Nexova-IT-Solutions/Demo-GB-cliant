...
export function SoftToysSection() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch("/api/products/soft-toys");
        if (!response.ok) {
          throw new Error("Failed to load soft toys");
        }
        const data = await response.json();
        setProducts(Array.isArray(data) ? data : []);
      } catch (error) {
        console.error("Failed to fetch soft toys:", error);
        setProducts([]);
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <section className="py-12 px-4 md:px-8 lg:px-10 max-w-[1600px] mx-auto">
      <SectionHeading
        title="Soft & Cuddly Toys"
        subtitle="Perfect companions for your gifts"
        showViewAll
